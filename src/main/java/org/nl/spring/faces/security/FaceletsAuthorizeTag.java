...
public class FaceletsAuthorizeTag extends AbstractAuthorizeTag {
     ...
     * @param ifAllGranted the ifAllAuthorityGranted attribute or null
     * @param ifAnyGranted the ifAnyAuthorityGranted attribute or null
     * @param ifNotGranted the ifNotAuthorityGranted attribute or null
     */
    public FaceletsAuthorizeTag(FaceletContext faceletContext, TagAttribute access, TagAttribute url,
                                ...
                                TagAttribute ifAllAuthorityGranted, TagAttribute ifAnyAuthorityGranted, TagAttribute ifNotAuthorityGranted) {
        ...
        setIfAllAuthorityGranted(getAttributeValue(faceletContext, ifAllAuthorityGranted, true));
        setIfAnyAuthorityGranted(getAttributeValue(faceletContext, ifAnyAuthorityGranted, true));
        setIfNotAuthorityGranted(getAttributeValue(faceletContext, ifNotAuthorityGranted, true));
    }
    ...
    void setIfAllAuthorityGranted(String ifAllGranted) {
        String[] roles = StringUtils.tokenizeToStringArray(ifAllGranted, ",");
        if (!ObjectUtils.isEmpty(roles)) {
            String expression = toHasAuthorityExpression(roles);
            setAccess(getAccess() != null ? getAccess() + " and " + expression: expression);
        }
    }

    void setIfAnyAuthorityGranted(String ifAnyGranted) {
        String[] roles = StringUtils.tokenizeToStringArray(ifAnyGranted, ",");
        if (!ObjectUtils.isEmpty(roles)) {
            String expression = toHasAnyAuthorityExpression(roles, false);
            setAccess(getAccess() != null ? getAccess() + " and " + expression : expression);
        }
    }

    void setIfNotAuthorityGranted(String ifNotGranted) {
        String[] roles = StringUtils.tokenizeToStringArray(ifNotGranted, ",");
        if (!ObjectUtils.isEmpty(roles)) {
            String expression = toHasAnyAuthorityExpression(roles, true);
            setAccess(getAccess() != null ? getAccess() + " and " + expression : expression);
        }
    }

    private static String toHasAuthorityExpression(String[] roles) {
        StringBuilder expression = new StringBuilder();
        boolean insertSeparator = false;
        for (String role : roles) {
            expression.append(insertSeparator ? " and " : "");
            expression.append("hasAuthority('").append(role).append("')");
            insertSeparator = true;
        }
        return expression.toString();
    }

    private static String toHasAnyAuthorityExpression(String[] roles, boolean negate) {
        StringBuilder expression = new StringBuilder();
        expression.append(negate ? "!" : "");
        expression.append("hasAnyAuthority(");
        boolean insertSeparator = false;
        for (String role : roles) {
            expression.append(insertSeparator ? "," : "");
            expression.append("'").append(role).append("'");
